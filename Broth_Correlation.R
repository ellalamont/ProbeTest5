...
source("Import_data.R") # to get Broth_tpm_CorrectScales
...
Broth_tpm_CorrectScales_Log10 <- Broth_tpm_CorrectScales %>%
  mutate(across(where(is.numeric), ~ .x + 1)) %>% # Add 1 to all the values
  mutate(across(where(is.numeric), ~ log10(.x))) # Log transform the values
...
my_path <- "Correlation_Figures/Broth_Correlations_CorrectScales"

for (i in 1:(length(colnames(Broth_tpm_CorrectScales_Log10)) -1 -1)) {
  for (j in (i + 1):(length(colnames(Broth_tpm_CorrectScales_Log10))-1)) {
    if (i != j) { # Avoid comparing the same samples or repeating comparisons
    # Access the samples
      Sample1 <- colnames(Broth_tpm_CorrectScales_Log10[i])
      Sample2 <- colnames(Broth_tpm_CorrectScales_Log10[j])
      # cat("Comparing:", Sample1, "with", Sample2, "\n")
      filename <- paste0(Sample1, "_ComparedTo_", Sample2, ".pdf")

      ScatterCorr <- Broth_tpm_CorrectScales_Log10 %>%
